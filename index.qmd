...
anim <- ggplot() +
  ...
  geom_text(data= subset(worldSquashed4, colr %in% "Ronaldo"), aes(label = paste0("Ronaldo's\n", goals), x = 130, y = 120), size = 10)+transition_states(Year, transition_length = 1, state_length = 15, T)
 
  
  
  
  
  # geom_text(data= subset(worldSquashed4, colr %in% "Ronaldo"), aes(label = paste0("Test\n", ifelse(year == "{closest_state}"), "Yes", "No"), x = 60, y = 120), size = 10)

  
  
  
  
  #stat_summary(data = subset(worldSquashed4, colr %in% "Messi"), aes(label = after_stat(goals), x = 100, y = 120), fun = 'sum', geom = 'text', col = 'black')
