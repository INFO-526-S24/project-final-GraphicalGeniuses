...
countryDataJoinedToMap <- joinCountryData2Map(combinedCopy, joinCode = "NAME", nameJoinColumn = "Country")
...
worldMessi <- subset(worldSquashed4, colr %in% "Messi") %>%
  group_by(Year) %>%
  summarise(goalsScored = sum(goals))

europeMessi <- subset(europeSquashed4, colr %in% "Messi") %>%
  group_by(Year) %>%
  summarise(goalsScored = sum(goals))
...
worldRonaldo <- subset(worldSquashed4, colr %in% "Ronaldo") %>%
  group_by(Year) %>%
  summarise(goalsScored = sum(goals))

europeRonaldo <- subset(europeSquashed4, colr %in% "Ronaldo") %>%
  group_by(Year) %>%
  summarise(goalsScored = sum(goals))
...
complexAnimationWorld <- ggplot() +
 ...
 geom_text(data = worldMessi, aes(label = paste0("Messi:\n", goalsScored)), x = -110, y = -40, size = 10)+
  geom_text(data = worldRonaldo, aes(label = paste0("Ronaldo:\n", goalsScored)), x = -150, y = -40, size = 10)+transition_states(Year, transition_length = 1, state_length = 40, T)
...
complexAnimationEurope <- ggplot() +
  ...
  ggtitle("Year: {closest_state}") +
  coord_sf(xlim = c(-10, 150), ylim = c(25, 75)) +
  theme(legend.position = "left",
        axis.title = element_blank(),
