...
The layers of the geographical maps will be the base map itself, of Europe as until 2022 that is where their careers have been based, that will be followed be a `fill` layer denoting that goals were scored in a specific country. The side-by-side animation will use a different `fill` colour for the two players, but the same European theater, to make it clear where each player has scored during each season. The animation speed will be taken into account, selecting an option that is slow enough to allow for direct inference of data and patterns, and could (potentially) be crafted in a way to allow for pausing. The seasons will be the same for the two maps at any point in the animation: for instance, the 2019-2020 season will display a few seconds after the 2018-2019 season, with both graphs changing at the same time.
